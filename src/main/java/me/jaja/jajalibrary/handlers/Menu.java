...
//    public void corner() {
//        setItem(0, JajaLibrary.itemInstance.create(Material.valueOf(JajaDungeons.getInstance().getConfig().getString("MenuCornerGlassItem"))));
//        setItem(8, JajaLibrary.itemInstance.create(Material.valueOf(JajaDungeons.getInstance().getConfig().getString("MenuCornerGlassItem"))));
//        setItem(45, JajaLibrary.itemInstance.create(Material.valueOf(JajaDungeons.getInstance().getConfig().getString("MenuCornerGlassItem"))));
//        setItem(53, JajaLibrary.itemInstance.create(Material.valueOf(JajaDungeons.getInstance().getConfig().getString("MenuCornerGlassItem"))));
//    }
...
//                setItem(i, JajaLibrary.itemInstance.create(Material.valueOf(JajaDungeons.getInstance().getConfig().getString("MenuGlassItem"))));
//        }
//
//    }

    public static ItemStack getPlayerHead(Player p) {
        ItemStack playerHead = JajaLibrary.itemInstance.create(Material.PLAYER_HEAD);
        SkullMeta sm = (SkullMeta) playerHead.getItemMeta();
        sm.setDisplayName(JajaLibrary.stringsInstance.format("PlayerHeadName", "%NAME%", p.getName(), p));
        sm.setOwner(p.getPlayer().getName());
        playerHead.setItemMeta(sm);
        playerHead = JajaLibrary.itemInstance.updateLore(playerHead, JajaLibrary.stringsInstance.formatList("PlayerHeadLore", p));
        return playerHead;
    }
    ...
    public static ItemStack getPlayerHeadWOLore(Player p) {
        ...
        SkullMeta sm = (SkullMeta) playerHead.getItemMeta();
//        sm.setDisplayName(
//                JajaLibrary.stringsInstance.colorText(
//                        instance.getLocalesInstance().getLocale().getString("PlayerHeadColor") + p.getName()
//                )
//        );
        sm.setDisplayName(JajaLibrary.stringsInstance.format("PartyMemberName", "%NAME%", p.getName(), p));
        sm.setOwner(p.getPlayer().getName());
        playerHead.setItemMeta(sm);
        return playerHead;
    }
    ...
    public void fill(){
        for (int i = 0; i < inventory.getSize(); i++) {
            ...
            setItem(i, JajaLibrary.itemInstance.create(Material.BLACK_STAINED_GLASS_PANE, ""));
        }
    }

    public static void corner(Inventory inventory, Material mat) {
        inventory.setItem(0, JajaLibrary.itemInstance.create(mat));
        inventory.setItem(8, JajaLibrary.itemInstance.create(mat));
        inventory.setItem(45, JajaLibrary.itemInstance.create(mat));
        inventory.setItem(53, JajaLibrary.itemInstance.create(mat));
    }

    public static void square(Inventory inventory, Material mat) {
        int rows = (inventory.getSize() + 1) / 9;
        for (int i = 0; i < rows * 9; i++) {
            if ((i <= 8) || (i >= (rows * 9) - 9) // bottom and top
                    || i == 9 || i == 18 // borders
                    ...
                    || i == 35 || i == 44)
//                if(inventory.getItem(i) != null) return;
                inventory.setItem(i, JajaLibrary.itemInstance.create(mat));
        }

    }

    public static void fill(Inventory inventory, Material mat){
        for (int i = 0; i < inventory.getSize(); i++) {
            if(inventory.getItem(i) != null) continue;
            inventory.setItem(i, JajaLibrary.itemInstance.create(mat));
        }
