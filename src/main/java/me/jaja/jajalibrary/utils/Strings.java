...
public class Strings {
    ...
    public List<String> formatList(String text) {
        List<String> textFromLocale = JajaLibrary.localesInstance.getLocale().getStringList(text);
    ...
    public List<String> formatList(String text, Player p) {
        ...
        for(String row : textFromLocale) {
            row = colorText(row);
            if(JajaLibrary.hooksInstance.usePlaceholderAPI) {
    ...
    public List<String> formatList(String text, String placeholder, String replace, Player p) {
        ...
        for(String row : textFromLocale) {
            ...
            if(placeholders.size() == replaces.size()){
                ...
                System.out.println(JajaLibrary.localesInstance.getLocale().getString("NotEnoughArgs") + " | " + placeholders + " " + replaces);
            }
    ...
    public String format(String text){
        if(JajaLibrary.localesInstance.getLocale().getString(text) == null) return "Not in localesInstance";
        text = JajaLibrary.localesInstance.getLocale().getString(text);

        text = text.replace("%prefix%", JajaLibrary.localesInstance.getLocale().getString("Prefix"));

        if (text.contains("%prefix%")) text = text.replace("%prefix%", JajaLibrary.localesInstance.getLocale().getString("Prefix") + "&f");

        return colorText(text);
    }

    public String format(String text, String placeholder, String replace){
        text = JajaLibrary.localesInstance.getLocale().getString(text) + "&7";
